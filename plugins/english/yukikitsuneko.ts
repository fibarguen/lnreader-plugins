...
class Yukikitsuneko implements Plugin.PluginBase {
    ...
    async popularNovels(): Promise<Plugin.NovelItem[]> {
        ...
        entries.forEach((element: FeedEntry) => {
            ...
            const link = (element.link ?? []).find((l: { rel?: string; href?: string }) => l.rel === "alternate")?.href ?? "#";
            const cover = element.media$thumbnail?.url || defaultCover;
            if (!title.toLowerCase().includes("series list") && !title.toLowerCase().includes("dmca")) {
                novels.push({
                    name: title,
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
        ...
        const hasTOCWidget = loadedCheerio('#series-toc-widget').length > 0;
        ...
        const novel: Plugin.SourceNovel = {
            path: novelPath,
            name: novelName,
            cover: loadedCheerio('.img-fluid').attr('src') || loadedCheerio('.col-md-6 img').attr('src') || defaultCover,
            artist: "Unknown",
        };

        const metadata = loadedCheerio('.col-12.col-md-6').eq(1);
        metadata.find('div').each(function () {
            const text = loadedCheerio(this).text().trim();
            const authorRegex = /^Author:\s*(.+)$/;
            const genderRegex = /^Genre:\s*(.+)$/;
            const statusRegex = /^Status:\s*(.+)$/;
            const authorMatch = authorRegex.exec(text);
            if (authorMatch) novel.author = authorMatch[1];
            const genderMatch = genderRegex.exec(text);
            if (genderMatch) novel.genres = genderMatch[1];
            const statusMatch = statusRegex.exec(text);
            if (statusMatch) novel.status = statusMatch[1].toLocaleLowerCase() === "on-going" ? NovelStatus.Ongoing : NovelStatus.Completed;
        });

        novel.summary = loadedCheerio('h5.text-info:contains("Description")')
            .parent()
            .find('p')
        ...
        const noveltitle = loadedCheerio("meta[property='og:title']").attr('content') || '';
        ...
        if (hasTOCWidget) {
                ...
                for (const entry of entries) {
                    ...
                    if (title != noveltitle) {
                        if (url && title && !seenUrls.has(url)) {
                            const labels = entry.category?.map((cat: { term?: string }) => cat.term) || [];
                            if (labels.some((item: string) => item.toLowerCase().includes('volume'))) {
            ...
            loadedCheerio("h6.mt-0 a").each((_, element) => {
                const chapterName = loadedCheerio(element).text().trim();
                const releaseTime = '';
                const chapterPath = loadedCheerio(element).attr('href')?.replace(this.site, '') || '';
    ...
    async searchNovels(searchTerm: string,): Promise<Plugin.NovelItem[]> {
        ...
        const url = `${this.site}feeds/pages/summary/-/Series` +
        ...
        novels = novels.filter(
            novel => novel.name.toLowerCase().includes(
